...
report_class = report_class_uat + report_class_dev + report_class_sit + report_class_prd + report_class_stc + report_class_batch + report_class_gtsa
report_class_tran = ('RCOFYD', 'RCOF6Y', 'RCVRSI', 'RCCWXN')
fcs_channels = ('44','45','54','55')
matrix_titile = (
                ('INTERVAL' , 'MVS_BSY%' , 'STOR_AVAL%', 'CSA%', 'SQA%', 'ECSA%', 'ESQA%', 'FCS_CHAN_ALL%') + tuple([i+'_APPL%' for i in report_class]) 
...
with open('DDS_REPORT_%s.D%s' %(sysname,day),'w') as outfile:
        ...
        stop = time.time()
        message = '\nDDS_REPORT_{sys}.D{d} saved in {dur} seconds.'.format(sys=sysname,d=day,dur=round(stop-start,1))
        print(message)
